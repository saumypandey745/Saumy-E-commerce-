...
const connectRabbitMQ = async () => {
    ...
    if (process.env.NODE_ENV === 'production') {
        try {
            ...
            console.error('[Order Service] RabbitMQ unavailable – exiting.');
            process.exit(1);
        }
    } else {
        if (useMock) {
            console.info('[Order Service] Running with In-Memory Mock Broker (forced).');
            channel = mockBroker;
            activeMode = 'mock';
        } else {
            try {
                ...
                console.warn('[Order Service] Real RabbitMQ unavailable – falling back to mock.');
                channel = mockBroker;
                activeMode = 'mock';
            }
        }
...
const getChannel = () => {
    if (!channel) {
        console.warn('[Order Service] Returning MOCKED RabbitMQ channel (lazy init)');
        return mockBroker;
    }
    return channel;
};
