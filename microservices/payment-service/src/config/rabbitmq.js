...
const connectRabbitMQ = async () => {
    ...
    if (process.env.NODE_ENV === 'production') {
        try {
            ...
            console.error('[Payment Service] RabbitMQ unavailable – exiting.');
            process.exit(1);
        }
    } else {
        if (useMock) {
            console.info('[Payment Service] Running with In-Memory Mock Broker (forced).');
            channel = mockBroker;
            activeMode = 'mock';
        } else {
            try {
                ...
                console.warn('[Payment Service] Real RabbitMQ unavailable – falling back to mock.');
                channel = mockBroker;
                activeMode = 'mock';
            }
        }
...
const publishEvent = (routingKey, message) => {
    if (!channel) {
        console.error('[Payment Service] RabbitMQ channel not initialized');
        return;
    }
...
const getChannel = () => {
    if (!channel) {
        console.warn('[Payment Service] Returning MOCKED RabbitMQ channel (lazy init)');
        return mockBroker;
    }
    return channel;
};
