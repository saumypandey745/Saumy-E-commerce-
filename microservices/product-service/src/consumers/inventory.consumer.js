...
const startInventoryConsumer = async () => {
    ...
    channel.consume(q.queue, async (msg) => {
        ...
        try {
            ...
            else if (routingKey === 'command.product.release_stock') {
                 console.log(`[Product Service] Received release_stock command for order ${content.order_id}`);
                 const mockReq = { body: { items: content.items, order_id: content.order_id } };
                 const mockRes = { status: () => ({ json: () => {} }) };
                 await productController.releaseStock(mockReq, mockRes, () => {});
            }
