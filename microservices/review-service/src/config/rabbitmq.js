...
const connectRabbitMQ = async () => {
    ...
    if (process.env.NODE_ENV === 'production') {
        try {
            ...
            console.log('[Review Service] RabbitMQ connected successfully.');
            activeMode = 'rabbitmq';
        } catch (error) {
            console.error('[Review Service] RabbitMQ unavailable – exiting.');
            process.exit(1);
        }
    } else {
        if (useMock) {
            console.info('[Review Service] Running with In-Memory Mock Broker (forced).');
            channel = mockBroker;
            activeMode = 'mock';
        } else {
            try {
                ...
                console.warn('[Review Service] Real RabbitMQ unavailable – falling back to mock.');
                channel = mockBroker;
                activeMode = 'mock';
            }
        }
...
const publishEvent = (routingKey, message) => {
    if (!channel) {
        console.error('[Review Service] RabbitMQ channel not initialized');
        return;
    }
...
const getChannel = () => {
    if (!channel) {
        console.warn('[Review Service] Returning MOCKED RabbitMQ channel (lazy init)');
        return mockBroker;
    }
    return channel;
};
