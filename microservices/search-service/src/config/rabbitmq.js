...
const connectRabbitMQ = async () => {
    ...
    if (process.env.NODE_ENV === 'production') {
        ...
        if (useMock) {
            console.info('[Search Service] Running with In-Memory Mock Broker (forced).');
            channel = mockBroker;
            activeMode = 'mock';
        } else {
            try {
                ...
                console.warn('[Search Service] Real RabbitMQ unavailable – falling back to mock.');
                channel = mockBroker;
                activeMode = 'mock';
            }
        }
...
const publishEvent = (routingKey, message) => {
    if (!channel) {
        console.error('[Search Service] RabbitMQ channel not initialized');
        return;
    }
...
const getChannel = () => {
    if (!channel) {
        console.warn('[Search Service] Returning MOCKED RabbitMQ channel (lazy init)');
        return mockBroker;
    }
    return channel;
};
